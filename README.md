...
- [ ] Implement/extract graph from coverage results
...
3. Execute fuzz generated inputs for gcov/bbcov coverage
    * Command to run coverage extraction on local
        ```
        $ time python3 fuzzer-xyz/src/scripts/get_cov.py [-h] -o OUTPUT_DIR -t TARGET_PROGRAM -f FUZZER -e EXPERIMENT_NAME -fid FUZZ_ID [-w WINDOW_SIZE] [--no-cleaup]
        ```
    * Command to run coverage extraction on server (distributed tasks)
        ```
        $ time python3 fuzzer-xyz/src/scripts/get_cov_on_server.py [-h] -o OUTPUT_DIR -f FUZZER -e EXPERIMENT_NAME [-p PARALLEL_TASKS] [-w WINDOW_SIZE]
        ```